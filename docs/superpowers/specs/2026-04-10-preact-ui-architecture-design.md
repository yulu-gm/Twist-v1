...
5. 最终迁完所有旧 DOM 面板，使旧 `adapter/ui/*` 退出主运行链路。
6. 按业务域优先级建立 UI 系统：
   - 第一优先：角色管理 `C`
   - 第二优先：建造规划 `B`
   - 第三优先：指令反馈 `A`
...
- 本次设计不要求在第一阶段一次性迁完所有旧 DOM 面板，但最终目标仍是全部迁移并退役 legacy UI。
...
允许新旧 UI 在迁移期共存，但新增业务能力优先进入新架构，不再继续扩展旧 `adapter/ui/*` 的直接耦合模式。迁移完成后，所有旧 DOM 面板都应被替换并退役。
...
迁移后建议新增 `Preact AppShell` 作为主 UI 根，旧 `dom-ui-manager` 在过渡期只承接尚未迁移的 legacy 面板。最终目标是删除或彻底退役这些 legacy 面板，使 `dom-ui-manager` 不再承担运行时 UI 组装职责。
...
### Phase 5：退役 Legacy DOM UI

- 将剩余旧 DOM 面板全部迁入 `Preact AppShell`
- 删除或清空 `dom-ui-manager` 的运行时装配职责
- 移除旧 `adapter/ui/*` 面板实现及其样式依赖
- 确认所有 UI 主入口均来自新 `src/ui/*` 架构
...
7. 整体迁移完成时，所有旧 DOM 面板都已被 `Preact` UI 替换并退役。
...
- 以完整迁移旧 DOM 面板为最终收尾，而不是长期维持双轨 UI
