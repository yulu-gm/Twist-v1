...
export type MockWorldPortConfig = Readonly<{
  /** 为 `true` 时对所有命令返回 accepted（仍写入日志）。 */
  alwaysAccept: boolean;
  /**
   * 将包含这些格键之一的命令标记为冲突拒绝，便于测拒绝路径。
   * 默认空集：不拒绝。
   */
...
export class MockWorldPort {
  ...
  public submit(raw: DomainCommand, nowMs: number): MockWorldSubmitResult {
    ...
    if (!this.config.alwaysAccept) {
      const rejected: MockWorldSubmitResult = {
        accepted: false,
        messages: ["mock: 已关闭 alwaysAccept"]
      };
      this.results.push(rejected);
      return rejected;
    }
    ...
    const ok: MockWorldSubmitResult = {
      accepted: true,
      messages: [`mock: 已接收 ${cmd.verb}，格数 ${cmd.targetCellKeys.length}`],
