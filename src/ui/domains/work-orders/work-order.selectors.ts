 ...
 * @description 工作订单领域选择器 — 从 EngineSnapshot + UiState 派生看板视图模型
...
/**
 * 把单个 WorkOrderNode 投影为列表行 — 同时格式化进度与活跃工人标签
 */
function toRow(node: WorkOrderNode): WorkOrderRow {
  return {
    id: node.id,
 ...
 * @returns 看板视图模型（rows + selectedOrderId + detail）
 */
export function selectWorkOrderBoard(
  ...
  const rows = workOrders.list.map(toRow);

  // 仅当 inspectorTargetId 命中订单 ID 时才视为有效选中
  const targetId = uiState.inspectorTargetId;
  const selectedNode = targetId ? workOrders.byId[targetId] : undefined;
  const selectedOrderId = selectedNode ? selectedNode.id : null;
  const detail = selectedNode ? toDetail(selectedNode) : null;

  return { rows, selectedOrderId, detail };
}
