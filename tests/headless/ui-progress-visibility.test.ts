 ...
 */
const ANCHOR_SEC = DEFAULT_SIM_CONFIG.workItemAnchorDurationSec;
...
function workBarVisibleOnMap(
  ...
  const totalSec = item !== undefined ? workItemAnchorDurationSeconds(ANCHOR_SEC, item.kind) : undefined;
  const showWorkBar =
    pawn.workTimerSec > 0 &&
    wid !== undefined &&
    item !== undefined &&
...
describe("UI-002 work bar visibility (headless)", () => {
  it("伐木：读条中途应对玩家可见进度条；树移除后条消失", () => {
    ...
    const total = workItemAnchorDurationSeconds(ANCHOR_SEC, itemMid!.kind);
